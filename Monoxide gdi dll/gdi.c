...
GdiThreadStart(INT number)
{
	...
	for (;;)
	{
		HANDLE hGdiThread = CreateThread(NULL, 0, (PTHREAD_START_ROUTINE)GdiShaderThread, &pGdiShaders[number], 0, NULL);
...
VOID AllGdiStart()
{
	...
	for ( ;; )
	{
		HANDLE hGdiThread = CreateThread(NULL, 0, (PTHREAD_START_ROUTINE)GdiShaderThread, &pGdiShaders[Xorshift32() % 21], 0, NULL);
		WaitForSingleObject(hGdiThread, (Xorshift32() % 3) ? PAYLOAD_MS : ((Xorshift32() % 5) * (PAYLOAD_MS / 4)));
		CloseHandle(hGdiThread);
